...
async def conectividad_terrestre_mendoza_regulares():
    ...
    async with async_playwright() as p:
        ...
        file_path = f"./descargas/conectividad_terrestre_mendoza_regulares.csv"
...
async def conectividad_terrestre_mendoza_turisticos():
    ...
    async with async_playwright() as p:
        ...
        await page.get_by_role("link", name="SERVICIOS DE TURISMO").click()

        fecha_container = page.locator(".input-daterange").last

        await fecha_container.click()
        ...
        await page.locator(".datepicker-months").get_by_text("Abr").click()

        await date_picker.locator("td.day:not(.old):not(.new)", has_text="1").first.click()

        input_fecha = page.locator(".input-daterange .form-control").first
        await expect(input_fecha).to_have_value("2019-04-01")
        ...
        # Guardamos el archivo con el nombre que nos sugiere el servidor
        file_path = f"/descargas/conectividad_terrestre_mendoza_turisticos.csv"
        await download.save_as(file_path)
...
async def join_conectividad_terrestre():
    ...
    df_turisticos = pd.read_csv("./descargas/conectividad_terrestre_mendoza_turisticos.csv", sep=",", low_memory=False)
    df_turisticos["Tipo"] = "SERVICIOS DE TURISMO"

    df_final = pd.concat([df_regulares, df_turisticos], axis=0)
